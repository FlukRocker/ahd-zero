...
<template>
    <Link :href="item.href" class="group block">
        <div ref="tiltEl" class="poster-card tilt">
            ...
            <span
                v-if="rank"
                class="absolute top-3 right-3 font-display italic leading-none"
                style="font-size: 56px; color: white; text-shadow: 0 4px 20px rgba(0, 0, 0, 0.6), 0 0 0 1px rgba(0, 0, 0, 0.4);"
            >#{{ rank }}</span>
            <div class="grad-bot" />
            <div class="absolute bottom-3 left-3 right-3 text-white">
                <div class="flex items-center gap-2 text-[11px] opacity-90 font-mono">
            ...
            >
                <span class="btn btn-primary"><AhdIcon name="play" :size="14" /> Watch</span>
            </div>
        </div>
        <div class="mt-3">
            <div class="flex items-center gap-2 mb-1">
                ...
                <span v-if="item.kanji" class="text-[11px] font-mono" style="color: hsl(var(--fg-faint));">{{ item.kanji }}</span>
            </div>
